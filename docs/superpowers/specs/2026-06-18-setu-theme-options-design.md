# Setu Theme Options Engine (sub-project #3c) — Design

> Slice **3c** of the render/theme epic. Parent vision:
> `docs/superpowers/specs/2026-06-17-setu-render-theme-vision.md`. 3a (default theme) ✅,
...
- **`themeOptions` config field** (`@setu/core`) — additive, optional
  `themeOptions?: Record<string, string>` on `SetuConfig` + `ResolvedConfig` + schema
...
- `types.ts`: `SetuConfig` + `ResolvedConfig` gain `themeOptions?: Record<string, string>`.
...
- **Font delivery is the one intentional visible change** — Google `<link>` → self-hosted
  ...
  and removes the GDPR/IP-leak liability every Setu-built site would otherwise inherit
...
See [[setu-project]], the parent vision doc, the 3a + 3b specs, and PRD §8.
