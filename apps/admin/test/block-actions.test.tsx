  ...
  const editor = useEditor({
    ...
    content: {
      ...
      content: [
        {
          type: 'callout',
          attrs: { mdAttrs: { type: 'warning', title: 'Heads up' } },
          content: [
...
describe('BlockActions', () => {
  it('keeps the caret in the moved paragraph when moving past a callout (up then down)', () => {
    ...
    }) // P -> index 1, back below the callout
    expect(editor.state.selection.$from.node(1).type.name).toBe('paragraph')
    expect(editor.state.selection.$from.node(1).textContent).toBe('P')
  })
  ...
  it('moveBlockDown reorders and round-trips to Markdoc', () => {
    ...
    expect(tiptapToMarkdoc(editor.getJSON())).toBe(
      tiptapToMarkdoc(docOf('B', 'A', 'C'))
    )
  })
  ...
  it('duplicateBlock preserves a callout mdAttrs and nested content', () => {
    ...
    expect(callouts[0]?.attrs?.mdAttrs).toEqual({
      type: 'warning',
      title: 'Heads up'
    })
    expect(callouts[1]?.attrs?.mdAttrs).toEqual({
      type: 'warning',
      title: 'Heads up'
    })
  })
...
describe('BlockActions — top-level atom (imageBlock)', () => {
  ...
  it('stays selected on the imageBlock across consecutive moves (down then up)', () => {
    ...
    const imgIndex = () =>
      (
        editor.getJSON() as { content: Array<{ type: string }> }
      ).content.findIndex((n) => n.type === 'imageBlock')
    selectImageBlock(editor)
    ...
    act(() => {
      editor.commands.moveBlockUp()
    }) // only moves the image back if the selection stuck
    expect(imgIndex()).toBe(1)
  })
})
