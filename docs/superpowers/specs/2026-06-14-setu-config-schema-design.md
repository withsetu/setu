...
- The authored config shape (`SetuConfig`, `BlockDefinition`) and a typed,
...
├── types.ts           # SetuConfig, BlockDefinition, ResolvedConfig, ResolvedBlock
...
- `defineConfig(config: SetuConfig): SetuConfig` — identity at runtime; exists
...
- `defaultConfig: SetuConfig` — the shipped default (one `callout` block).

New types exported from `@setu/core`: `SetuConfig`, `BlockDefinition`,
