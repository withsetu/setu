...
describe('EditorScreen', () => {
  ...
  it('opens a blank canvas for an absent entry', async () => {
    const services = fakeServices({
      read: {
        loadForEdit: vi.fn(async () => ({ source: 'absent' }))
      } as unknown as Services['read']
    })
  ...
  it('renders read-only with a banner when the lock is blocked', async () => {
    const services = fakeServices()
    ;(
      services.authoring.open as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce({
      granted: false,
      outcome: 'blocked',
      lock: { ...aLock, lockedBy: 'someone' },
      draft: aDraft
    })
    renderEditor(services)
    expect(
      await screen.findByText(/locked by another editor/i)
    ).toBeInTheDocument()
  })
  ...
  it('persists across a reopen (real services)', async () => {
    ...
    expect(
      await screen.findByDisplayValue('Release notes v2')
    ).toBeInTheDocument()
  })
