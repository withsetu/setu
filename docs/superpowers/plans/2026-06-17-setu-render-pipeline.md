...
**Goal:** Build `apps/site`, an Astro app that renders every shipped editor block to correct static HTML from committed sample `.mdoc` content — the first time Setu's authored content becomes a visible website.
...
**1. Spec coverage** (against `2026-06-17-setu-render-pipeline-design.md`):
