...
describe('reprocess runner', () => {
  ...
  it('counts only reprocessed images — a skipped key advances the cursor but not the count', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'rpr-mix-')); dirs.push(dir)
    const storage = createLocalStorage({ dir, baseUrl: 'http://localhost:4444/media' })
    const image = createSharpImageAdapter()
    ...
    const goneManifest: MediaManifest = {
      id: '2026/01/gone', format: 'webp',
      original: { key: '2026/01/gone.png', width: 400, height: 300, format: 'png' }, variants: [],
    }
    await storage.put(manifestKey('2026/01/gone'), new TextEncoder().encode(JSON.stringify(goneManifest)), { contentType: 'application/json' })
