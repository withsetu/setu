...
git commit -m "feat(git-http): browser-side GitPort adapter over the Setu git API (#bridge)
...
- After merge: update `memory/setu-project.md` + `docs/roadmap.md` (mark the Local Bridge / Cut A shipped; next = Cut B server-side drafts/locks, or the `git-github`/edge adapters; this also closed the "single dev command" item).
