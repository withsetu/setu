...
export function QueryPreview({
  ...
  return (
    <div
      className="not-prose my-2 rounded-lg border border-border bg-muted/30"
    ...
    >
      ...
      <div className="p-3">
        {state === 'ready' && rows.length === 0 ? (
          ...
          >
            {(state === 'loading'
              ? Array.from({ length: Math.min(columns, 3) }, () => null)
              : rows
            ).map((row, i) => (
              <li
                key={
                  row
                    ? `${row.ref.collection}/${row.ref.locale}/${row.ref.slug}`
                    : `sk-${i}`
                }
                className={
                  layout === 'list'
