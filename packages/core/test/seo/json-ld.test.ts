...
import { resolveJsonLd, jsonLdScript, type JsonLdInput } from '../../src/seo/json-ld'
import { DEFAULT_SETTINGS } from '../../src/settings/defaults'
import type { SiteSettings } from '../../src/settings/types'

const settings = (over: Partial<SiteSettings['identity']> = {}, general: Partial<SiteSettings['general']> = {}): SiteSettings => ({
  ...DEFAULT_SETTINGS,
...
describe('resolveJsonLd', () => {
  ...
  it('uses a Person entity when identity.entityType is person', () => {
    const g = resolveJsonLd(settings({ entityType: 'person', name: 'Ada' }), input())
  ...
  it('includes sameAs + Organization logo when present', () => {
    ...
    expect((org.logo as { url: string }).url).toBe('https://example.com/logo.png')
  })

  it('adds an Article node (with dates, image ref, author/publisher) for posts', () => {
    const g = resolveJsonLd(
      settings(),
      input({ type: 'article', image: 'https://example.com/i.jpg', datePublished: '2026-06-20T00:00:00.000Z', dateModified: '2026-06-21T00:00:00.000Z' }),
    )
    ...
    expect(art.mainEntityOfPage).toEqual({ '@id': 'https://example.com/post/hello/#webpage' })
    expect(art.author).toEqual({ '@id': 'https://example.com/#organization' })
    expect(art.image).toEqual({ '@id': 'https://example.com/post/hello/#primaryimage' })
    // the primary image object is present and shared by @id
    expect(node(g, 'ImageObject')!['@id']).toBe('https://example.com/post/hello/#primaryimage')
  })

  it('WebPage name falls back to the site name on the homepage (empty pageTitle)', () => {
    const g = resolveJsonLd(settings({}, { title: 'Setu Press' }), input({ pageTitle: '', canonical: 'https://example.com/' }))
...
describe('jsonLdScript', () => {
  it('escapes < so a value cannot break out of the script tag', () => {
    const g = resolveJsonLd(settings({}, { description: 'evil </script><script>alert(1)' }), input())
