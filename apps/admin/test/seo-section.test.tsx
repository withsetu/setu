...
function renderSeo(metadata: Record<string, unknown> = { title: 'My Post' }) {
  ...
  render(<SeoSection metadata={metadata} slug="my-post" editable onChange={onChange} apiBase="" />)
  return { onChange }
}
...
describe('SeoSection', () => {
  it('writes the SEO title override under metadata.seo', () => {
    ...
    fireEvent.change(screen.getByLabelText('SEO title'), { target: { value: 'Custom SEO Title' } })
    expect(onChange).toHaveBeenLastCalledWith(
      expect.objectContaining({ title: 'My Post', seo: { title: 'Custom SEO Title' } }),
    )
  })

  it('writes the meta description override', () => {
    const { onChange } = renderSeo()
    fireEvent.change(screen.getByLabelText('Meta description'), { target: { value: 'A custom description.' } })
    expect(onChange).toHaveBeenLastCalledWith(
  ...
  it('noindex toggle (under Advanced) sets seo.noindex true', () => {
    ...
    expect(onChange).toHaveBeenLastCalledWith(expect.objectContaining({ seo: { noindex: true } }))
  })
