...
export interface MediaIndexService {
  ...
  query(
    q: MediaIndexQuery
  ): Promise<{ rows: import('./types').MediaIndexRow[]; total: number }>
...
export function createMediaIndexService({
  mediaIndex,
  fetchRaw
}: MediaIndexServiceDeps): MediaIndexService {
