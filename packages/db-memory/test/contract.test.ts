...
describe('createMemoryDataPort seed', () => {
  it('applies seed drafts so listDrafts returns them', async () => {
    ...
    expect((await db.listDrafts()).map((d) => d.slug).sort()).toEqual([
      'a',
      'b'
    ])
    expect(
      (await db.listDrafts({ collection: 'post' })).map((d) => d.slug)
    ).toEqual(['a'])
    expect(
      (await db.getDraft({ collection: 'page', locale: 'en', slug: 'b' }))
        ?.metadata
    ).toEqual({ title: 'B' })
  })
  ...
  it('does not collide when fields contain the separator char (space)', async () => {
    ...
    expect(
      (await db.getDraft({ collection: 'a b', locale: 'en', slug: 'x' }))
        ?.metadata
    ).toEqual({ n: 1 })
    expect(
      (await db.getDraft({ collection: 'a', locale: 'b en', slug: 'x' }))
        ?.metadata
    ).toEqual({ n: 2 })
  })
  ...
  it('isolates stored drafts from later mutation of the input objects', async () => {
    ...
    expect(
      (await db.getDraft({ collection: 'post', locale: 'en', slug: 'm' }))
        ?.metadata
    ).toEqual({ title: 'Orig' })
  })
})
