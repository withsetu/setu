...
describe('mintSlug', () => {
  it('derives a slug from the title', async () => {
    const slug = await mintSlug(createMemoryDataPort([]), createMemoryGitPort([]), 'post', 'en', 'Post Test')
    expect(slug).toBe('post-test')
  })

  it('avoids an existing DRAFT slug in the same collection+locale', async () => {
    const seed: DraftInput[] = [{ collection: 'post', locale: 'en', slug: 'hello', content: doc('x'), metadata: {} }]
    const slug = await mintSlug(createMemoryDataPort(seed), createMemoryGitPort([]), 'post', 'en', 'Hello')
    expect(slug).toBe('hello-2')
  })

  it('avoids an existing COMMITTED slug', async () => {
    const git = createMemoryGitPort([
      { path: 'content/post/en/hello.mdoc', content: serializeMdoc({ frontmatter: { title: 'Hello' }, body: 'hi' }) },
    ])
    const slug = await mintSlug(createMemoryDataPort([]), git, 'post', 'en', 'Hello')
    expect(slug).toBe('hello-2')
  })

  it('does not collide across a different locale', async () => {
    const seed: DraftInput[] = [{ collection: 'post', locale: 'fr', slug: 'hello', content: doc('x'), metadata: {} }]
    const slug = await mintSlug(createMemoryDataPort(seed), createMemoryGitPort([]), 'post', 'en', 'Hello')
    expect(slug).toBe('hello')
  })

  it('reserves the `new` sentinel', async () => {
    const slug = await mintSlug(createMemoryDataPort([]), createMemoryGitPort([]), 'post', 'en', 'New')
