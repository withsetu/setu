      ...
      email:
        notifyFrom && adminOrigin
          ? {
              ...
              // #499: resolve the BODY at send time too, through the same live template
              // resolver every other send path uses. @setu/auth hands over the reset link it
              // built (and callback-defaulted) — the template can place `{{reset_url}}` but
              // never supply or alter one.
