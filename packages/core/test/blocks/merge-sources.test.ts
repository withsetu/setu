...
const std: StandardBlock[] = [
  {
    ...
    contract: defineBlock({ props: z.object({ href: z.string() }) })
  }
]
...
describe('mergeBlockSources', () => {
  ...
  it('lets a site-local folder block override a standard block by tag', () => {
    const localContract = defineBlock({
      props: z.object({ href: z.string(), extra: z.string().optional() })
    })
    const out = mergeBlockSources({
      ...
      local: [
        {
          tag: 'button',
          component: 'blocks/button/button.astro',
      ...
      local: [
        {
          ...
          contract: defineBlock({ props: z.object({}) })
        }
      ]
    })
