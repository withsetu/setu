...
Then ADD these app-specific rules (Tiptap renders semantic tags inside `.saytu-prose`, so map the design's per-block typography onto the real tags; values mirror `design/admin/editor.css`'s `.blk-*`):
```css
/* Tiptap renders semantic tags; map the design's block typography onto them. */
.saytu-prose { outline: none; }
.saytu-prose p { font-size: 19px; line-height: 1.75; color: var(--text); padding: 5px 0; letter-spacing: -.003em; margin: 0; }
.saytu-prose h1 { font-size: 31px; line-height: 1.25; font-weight: 700; letter-spacing: -.02em; margin: 30px 0 8px; color: var(--text); }
.saytu-prose h2 { font-size: 25px; line-height: 1.3; font-weight: 700; letter-spacing: -.018em; margin: 28px 0 6px; color: var(--text); }
.saytu-prose h3 { font-size: 20px; line-height: 1.35; font-weight: 650; letter-spacing: -.01em; margin: 20px 0 4px; color: var(--text); }
.saytu-prose ul, .saytu-prose ol { margin: 6px 0; padding-left: 26px; }
.saytu-prose li { font-size: 19px; line-height: 1.7; padding: 2px 0; color: var(--text); }
.saytu-prose blockquote { font-size: 22px; line-height: 1.55; font-style: italic; color: var(--text); padding: 4px 0 4px 22px; border-left: 3px solid var(--accent); margin: 8px 0; }
.saytu-prose pre { background: var(--bg-sunken); border: 1px solid var(--border); border-radius: var(--r-sm); padding: 14px 16px; font-size: 14px; line-height: 1.6; color: var(--text); white-space: pre-wrap; font-family: var(--font-mono); margin: 10px 0; }
.saytu-prose code { font-family: var(--font-mono); font-size: .9em; }
.saytu-prose hr { border: none; height: 1px; background: var(--border-strong); margin: 18px 0; }
.saytu-prose a { color: var(--accent-strong); text-decoration: underline; }
/* Placeholder (from @tiptap/extension-placeholder). */
.saytu-prose p.is-editor-empty:first-child::before { content: attr(data-placeholder); color: var(--text-4); float: left; height: 0; pointer-events: none; }
