...
function makeWrapper(data = createMemoryDataPort(), git = createMemoryGitPort(), index = createMemoryIndexPort()) {
...
describe('TaxonomyProvider', () => {
  ...
  it('remove() strips the category from categories and refreshes counts', async () => {
    ...
    const data = createMemoryDataPort([
      { collection: 'post', locale: 'en', slug: 'a', content: doc('a') as any, metadata: { title: 'A', categories: ['eng'] } },
    ])
    const git = createMemoryGitPort()

    // Seed categories.yaml so the deleter can find 'eng'
    const catsYaml = serializeCategories([{ slug: 'eng', name: 'Engineering', parent: null }])
    ...
    await waitFor(() => expect(result.current.categories.some((c) => c.slug === 'eng')).toBe(true))
