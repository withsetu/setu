...
**Goal:** Give the Setu site a real, designed look — one typographic identity, a header/footer shell, and Post (narrow) + Page (wider) templates — built from tokens-with-defaults so it's fully customization-ready.
...
```css
/* Setu default theme — token layer. Every taste choice is a knob with a default;
...
**1. Spec coverage** (against `2026-06-18-setu-default-theme-design.md`):
