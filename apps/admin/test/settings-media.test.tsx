  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: string) => {
      if (String(url).includes('/api/capabilities')) {
        return new Response(JSON.stringify({ capabilities: caps }), {
          status: 200
        })
      }
      return new Response(JSON.stringify({ reprocessed: reprocessedCount }), {
        status: 200
      })
    })
  )
...
describe('MediaSettings', () => {
  ...
  it('shows the reprocess button and local-run warning dialog', async () => {
    ...
    const localRunWarning = await screen.findByText(
      /best run locally, not on a deployed site/i
    )
  ...
  describe('topology gating', () => {
    ...
    it('disables Reprocess and shows uploads note when imageProcessing and writableMediaStore are false', async () => {
      ...
      const uploadsNote = await screen.findByText(
        /uploads won't generate variants/i
      )
  ...
  describe('async reprocess progress', () => {
    it('starts reprocess, polls, shows progress bar, and toasts the count on done', async () => {
      const fetchMock = vi
        ...
        .mockResolvedValue(
          new Response(
            JSON.stringify({ status: 'done', processed: 3, total: 3 }),
            { status: 200 }
          )
        )
