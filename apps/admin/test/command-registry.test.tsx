...
describe('command registry', () => {
  ...
  it('unregisters on unmount', () => {
    function Host() {
      ...
      useEffect(() => {
        setOn(false)
      }, []) // unmount the registrar after first paint
      return (
        <>
          {on && <Registrar actions={[act1]} />}
          <Probe />
        </>
      )
    }
  ...
  it('run delegates to the latest closure (no stale capture)', () => {
    ...
    function Counter() {
      ...
      useRegisterCommands([
        { id: 'c', title: 'C', group: 'G', run: () => calls.push(n) }
      ])
      return null
    }
