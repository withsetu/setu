# Setu Theme System (sub-project #3b) — Design
...
- **`theme` field in `setu.config`** (`@setu/core`): an additive, optional `theme?: string`
  (the active theme's package name) on `SetuConfig` + the schema + `ResolvedConfig`
...
- `types.ts`: `SetuConfig` gains `theme?: string`; `ResolvedConfig` gains `theme?: string`.
...
See [[setu-project]], the parent vision doc, the 3a spec, and PRD §8.
