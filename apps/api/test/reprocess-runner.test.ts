...
describe('reprocess runner', () => {
  // Sharp encodes real AVIF/WebP + LQIP here; the CPU-bound setup + reprocess races the
  // 5s default under CI parallelism (same class #478 fixed for media-upload). Give it a
  // timeout that fits the work rather than retrying a non-flaky computation. Setu #548.
  it(
    'resumes from the job cursor and upgrades only the remaining manifests',
    { timeout: 30_000 },
    async () => {
      ...
      expect(new Set(m1.variants.map((v) => v.format))).toEqual(
        new Set(['webp'])
      )
    }
  )
  ...
  it(
    'counts only reprocessed images — a skipped key advances the cursor but not the count',
    { timeout: 30_000 },
    async () => {
      ...
      await runReprocessJob(store, deps, job.id, {
        chunkSize: 10,
        now: () => 1
      })
