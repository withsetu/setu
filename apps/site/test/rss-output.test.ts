...
async function renderFeed(
  over: Partial<Parameters<typeof buildFeed>[0]> = {}
): Promise<string> {
  const res = await rss(
    buildFeed({
      title: 'My Site',
      ...
      items: [
        item({
          ...
          image: '/media/2026/06/one.jpg'
        }),
        item({
          title: 'Post Two',
          link: '/post/two',
          pubDate: new Date('2024-02-09T00:00:00Z')
        })
      ],
      ...over
    })
...
describe('rss output (serialized)', () => {
  ...
  it('emits language, generator (with version), atom:self and lastBuildDate', async () => {
    ...
    expect(xml).toContain(
      '<atom:link href="https://example.dev/rss.xml" rel="self" type="application/rss+xml"'
    )
    // newest of the two items
    expect(xml).toContain(
      '<lastBuildDate>Fri, 09 Feb 2024 00:00:00 GMT</lastBuildDate>'
    )
  })
  ...
  it('emits per-item categories and a media:content for the featured image', async () => {
    ...
    expect(xml).toMatch(
      /<media:content url="https?:\/\/[^"]*\/media\/2026\/06\/one\.jpg" medium="image" type="image\/jpeg"/
    )
  ...
  it('passes an external featured-image URL through unchanged (e.g. seeded recipe thumbnails)', async () => {
    const xml = await renderFeed({
      items: [
        item({ link: '/post/ext', image: 'https://cdn.example.com/thumb.png' })
      ]
    })
    expect(xml).toContain(
      '<media:content url="https://cdn.example.com/thumb.png" medium="image" type="image/png"'
    )
  })
  ...
  it('uses the locale for a per-locale feed', async () => {
    ...
    expect(xml).toContain(
      '<atom:link href="https://example.dev/fr/rss.xml" rel="self"'
    )
  })
