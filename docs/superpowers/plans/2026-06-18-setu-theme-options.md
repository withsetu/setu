...
**Files:**
- Modify: `packages/core/src/config/types.ts` (`SetuConfig`, `ResolvedConfig`)
...
**Interfaces:**
- Produces: `SetuConfig.themeOptions?: Record<string, string>`, `ResolvedConfig.themeOptions?: Record<string, string>`. `resolveConfig(raw).themeOptions` passes the authored value through (or `undefined`).
...
In `packages/core/src/config/types.ts`, add to `SetuConfig` (right after the `theme?: string` field):
...
**Interfaces:**
- Consumes: `optionsToCss` from `@setu/theme-default/options` (Task 2); `SetuConfig.themeOptions` (Task 1).
...
- After merge: update `memory/setu-project.md` + `docs/roadmap.md` (mark #3c shipped; next = the visual Customizer panel, gated on the editor→disk bridge).
