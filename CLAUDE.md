...
- CI (`.github/workflows/ci.yml`): PR = affected-only via turbo `...[merge-base]` (docs-only PRs
  skip; root/config changes run full); push to main + nightly (03:17 UTC) = full + e2e full
  matrix (`E2E_FULL_MATRIX=1`). CodeQL gates on NEW findings vs `.github/codeql-known-findings.json`
  (every entry has a tracking issue). `pnpm audit --audit-level=high` gates supply chain.
