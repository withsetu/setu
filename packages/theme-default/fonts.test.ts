...
import { FONT_PACKAGES, MONO_FONT_PACKAGE, DEFAULT_FONT, fontPackagesFor } from './fonts'
import { themeOptions } from './options'

describe('fontPackagesFor', () => {
  it('bundles the selected family + the mono font', () => {
    expect(fontPackagesFor('inter')).toEqual([FONT_PACKAGES.inter, MONO_FONT_PACKAGE])
  })
...
describe('font registry stays in sync with the theme option', () => {
  ...
  it('every `font` choice maps to a fontsource package (no drift)', () => {
    const missing = (fontOption.choices ?? []).map((c) => c.value).filter((v) => !(v in FONT_PACKAGES))
    expect(missing, `font choices missing a package: ${missing.join(', ')}`).toEqual([])
  })
