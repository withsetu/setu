...
const cssFiles = globSync('**/*.css', { cwd: SRC })
  .filter((f) => f !== 'tokens.css') // the base layer is allowed to define literals
...
describe('block CSS obeys the token contract', () => {
  it.each(cssFiles)('%s reads only contract tokens or --blk-* locals', (rel) => {
