...
import {
  addCategory,
  renameLabel as renameLabelOp,
  reparent as reparentOp
} from './ops'
...
export interface TaxonomyService {
  ...
  create(input: {
    name: string
    parent: string | null
  }): Promise<{ categories: Category[]; slug: string }>
...
export function createTaxonomyService(deps: {
  git: GitPort
  author: GitAuthor
}): TaxonomyService {
  ...
  return {
    ...
    async renameLabel(slug, name) {
      return commit(
        renameLabelOp(await read(), slug, name),
        `taxonomy: rename ${slug}`
      )
    },
    async reparent(slug, parent) {
      return commit(
        reparentOp(await read(), slug, parent),
        `taxonomy: reparent ${slug}`
      )
    }
  }
}
