...
function Icon({ name, size = 18, stroke = 1.75, className = '', style = {} }) {
  ...
  return (
    <svg className={className} width={size} height={size} viewBox="0 0 24 24" fill="none"
      ...
      dangerouslySetInnerHTML={{ __html: d }} aria-hidden="true" />
  );
}
...
/* ============================================================
   BUTTON
   ============================================================ */
function Button({ variant = 'default', size = 'md', icon, iconRight, children, className = '', as: Tag = 'button', ...rest }) {
  const cls = `btn btn-${variant} btn-${size}${children ? '' : ' btn-icononly'} ${className}`;
  return (
...
/* ============================================================
   ...
   ============================================================ */
function Badge({ tone = 'neutral', soft = true, dot = false, children, className = '', ...rest }) {
  return (
    <span className={`badge badge-${tone}${soft ? ' badge-soft' : ''} ${className}`} {...rest}>
...
function StatusPill({ status, lifecycle = false, size = 'md' }) {
  const s = STATUS_MAP[status] || STATUS_MAP.Draft;
  if (lifecycle) {
    const steps = ['Draft', 'Staged', 'Deployed'];
    const at = steps.indexOf(status);
    return (
      <div className="lifecycle" role="img" aria-label={`Status: ${status}`}>
        {steps.map((st, i) => (
          <React.Fragment key={st}>
            <span className={`lifecycle-node ${i <= at ? 'on' : ''} ${i === at ? 'cur' : ''}`}>
              <span className="lifecycle-dot" />{st}
            </span>
            {i < steps.length - 1 && <span className={`lifecycle-bar ${i < at ? 'on' : ''}`} />}
          </React.Fragment>
        ))}
      </div>
    );
  }
  return <Badge tone={s.tone} dot className={`pill-${size}`}>{s.label}</Badge>;
}

/* ============================================================
   PRO LOCK — reusable upsell pattern
   ============================================================ */
function ProChip({ label = 'Pro', size = 'sm' }) {
  return (
    <span className={`prochip prochip-${size}`}>
      <Icon name="sparkle" size={size === 'sm' ? 11 : 13} />{label}
    </span>
  );
}

function ProLock({ icon = 'zap', title, blurb, inline = false, onLearn }) {
  if (inline) {
    return (
      <button className="prolock-inline" onClick={onLearn} title={`${title} — available on Pro`}>
        <span className="prolock-inline-ic"><Icon name={icon} size={14} /></span>
        <span className="prolock-inline-label">{title}</span>
        <span className="prolock-inline-lock"><Icon name="lock" size={12} /> Pro</span>
      </button>
    );
  }
  return (
    <div className="prolock-card">
      <div className="prolock-card-head">
        <span className="prolock-card-ic"><Icon name={icon} size={16} /></span>
        <ProChip />
      </div>
      <div className="prolock-card-title">{title}</div>
      {blurb && <div className="prolock-card-blurb">{blurb}</div>}
      <button className="prolock-card-cta" onClick={onLearn}>Unlock with Pro <Icon name="arrowRight" size={14} /></button>
    </div>
  );
}

/* ============================================================
   FORM CONTROLS (admin forms)
   ============================================================ */
function Field({ label, hint, htmlFor, children, optional, action }) {
  return (
    <div className="field">
      {label && (
        <div className="field-head">
          <label className="field-label" htmlFor={htmlFor}>{label}{optional && <span className="field-opt">Optional</span>}</label>
          {action}
        </div>
      )}
      {children}
...
function Input({ icon, prefix, className = '', ...rest }) {
  return (
    <div className={`input-wrap ${icon ? 'has-icon' : ''} ${prefix ? 'has-prefix' : ''} ${className}`}>
...
function Toggle({ checked, onChange, label, id }) {
  return (
    <button type="button" id={id} role="switch" aria-checked={checked} aria-label={label}
      ...
      {options.map(o => {
        ...
        return (
          <button key={val} role="tab" aria-selected={value === val}
            className={`seg-item ${value === val ? 'on' : ''}`} onClick={() => onChange(val)}>{lab}</button>
        );
      })}
    </div>
  );
...
/* ============================================================
   AVATAR
   ============================================================ */
const AV_COLORS = ['#6366f1', '#0ea5e9', '#10b981', '#f59e0b', '#ef4444', '#ec4899', '#8b5cf6', '#14b8a6'];
function Avatar({ name = '?', size = 26, src, ring = false }) {
  const initials = name.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase();
  const hue = AV_COLORS[(name.charCodeAt(0) + (name.charCodeAt(name.length - 1) || 0)) % AV_COLORS.length];
  return (
    <span className={`avatar ${ring ? 'avatar-ring' : ''}`} style={{ width: size, height: size, fontSize: size * 0.4, background: src ? 'none' : `linear-gradient(150deg, ${hue}, color-mix(in oklch, ${hue} 70%, #000 18%))` }}>
...
/* ============================================================
   TOOLTIP (lightweight, CSS-driven)
   ============================================================ */
function Tip({ label, children, side = 'top' }) {
  return (
    <span className="tip-wrap" data-side={side}>
      {children}
...
/* ============================================================
   POPOVER / MENU — anchored, click-away
   ============================================================ */
function useClickAway(ref, onAway, active = true) {
  useEffect(() => {
    if (!active) return;
    const h = (e) => { if (ref.current && !ref.current.contains(e.target)) onAway(e); };
    const k = (e) => { if (e.key === 'Escape') onAway(e); };
...
function Menu({ items, onClose, style }) {
  ...
  return (
    <div className="menu" ref={ref} style={style} role="menu">
      {items.map((it, i) => it.sep
        ? <div key={i} className="menu-sep" />
        : (
          <button key={i} role="menuitem" className={`menu-item ${it.danger ? 'danger' : ''} ${it.disabled ? 'is-disabled' : ''}`}
            onClick={() => { if (it.disabled) return; it.onClick && it.onClick(); onClose(); }} disabled={it.disabled}>
...
/* ============================================================
   EMPTY STATE
   ============================================================ */
function EmptyState({ icon, title, blurb, action, secondary, mini = false }) {
  return (
    <div className={`empty ${mini ? 'empty-mini' : ''}`}>
      <div className="empty-art" aria-hidden="true">
        <div className="empty-art-grid" />
        <span className="empty-art-ic"><Icon name={icon} size={mini ? 22 : 28} /></span>
      </div>
      <div className="empty-title">{title}</div>
      {blurb && <div className="empty-blurb">{blurb}</div>}
...
Object.assign(window, {
  Icon, ICONS, Button, Badge, StatusPill, STATUS_MAP, ProChip, ProLock,
  Field, Input, Textarea, Toggle, Segmented, Avatar, Tip, Menu, useClickAway,
  EmptyState, Kbd,
});
