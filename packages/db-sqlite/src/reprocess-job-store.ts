...
export function createSqliteReprocessJobStore(file: string): ReprocessJobStore {
  ...
  return {
    create(keys, now) {
      ...
      db.prepare(
        `INSERT INTO reprocess_jobs (id,total,processed,cursor,status,error,keys,startedAt,updatedAt)
    ...
    active() {
      const r = db
        .prepare(
          "SELECT * FROM reprocess_jobs WHERE status = 'running' ORDER BY startedAt DESC"
        )
    ...
    latest() {
      const r = db
        .prepare('SELECT * FROM reprocess_jobs ORDER BY startedAt DESC')
        .get() as Row | undefined
      return r ? toJob(r) : null
    },
    saveProgress(id, processed, cursor, now) {
      db.prepare(
        'UPDATE reprocess_jobs SET processed=?, cursor=?, updatedAt=? WHERE id=?'
      ).run(processed, cursor, now, id)
    },
    finish(id, status, now, error) {
      db.prepare(
        'UPDATE reprocess_jobs SET status=?, error=?, updatedAt=? WHERE id=?'
