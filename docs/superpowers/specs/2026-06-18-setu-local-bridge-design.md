# Setu Local Bridge (editor→disk, Cut A) — Design
...
- **Edge / Cloudflare (Worker, `db-d1`, GitHub API), auth/tokens, the productized "user repo depends
  on Setu" restructuring, SSR preview, the "View Page" links.** All later.
...
- **No submodule** for content (decided): a submodule pins a SHA and would rebuild stale content;
  same-repo `content/` now, "user-repo-depends-on-Setu" later.

---

See [[setu-project]], the roadmap's "editor→disk bridge / multi-topology" section, and PRD §2.
