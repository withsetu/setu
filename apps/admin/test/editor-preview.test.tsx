...
describe('EditorScreen preview', () => {
  ...
  it('compiles the current draft, POSTs it to the api, and opens the site preview', async () => {
    ...
    const openMock = vi.fn().mockReturnValue({ closed: false, focus: vi.fn(), location: { href: '' } })
    ...
    expect(body).toMatchObject({ collection: 'post', locale: 'en', slug: 'release-notes' })
    expect(typeof body.content).toBe('string') // a compiled .mdoc
    await waitFor(() => expect(openMock).toHaveBeenCalled())
    expect(openMock.mock.calls[0]![0] as string).toContain('http://localhost:4321/preview')
