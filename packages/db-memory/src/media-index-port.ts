...
export function createMemoryMediaIndexPort(): MediaIndexPort {
  ...
  return {
    async query(q: MediaIndexQuery) {
      return runMediaQuery([...rows.values()], q)
    },
    async upsert(row) {
      rows.set(row.mediaKey, structuredClone(row))
    },
    async upsertMany(rs) {
      for (const r of rs) rows.set(r.mediaKey, structuredClone(r))
    },
    async remove(mediaKey) {
      rows.delete(mediaKey)
    },
    async clear() {
      rows.clear()
    },
