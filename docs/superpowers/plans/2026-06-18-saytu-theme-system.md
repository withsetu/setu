...
**Goal:** Make the default theme an installable, config-activated package — extract its look into `@saytu/theme-default`, add a `theme` field to `saytu.config`, and wire the site build to render through whichever theme the config names; the site looks **identical** to today.

**Architecture:** A new `packages/theme-default` holds the theme's `.astro` layouts + CSS (moved from `apps/saytu-site`). `@saytu/core`'s config gains an additive `theme` field. `apps/saytu-site/astro.config.mjs` reads `saytu.config`'s `theme` (via core's Node `loadConfig`, jiti) and sets a Vite alias `@theme` → the active theme package; the pages import layouts from `@theme/…`. Render engine (routing/Markdoc/block components) stays in the app.

**Tech Stack:** Astro 6 · a new `@saytu/theme-default` package (`.astro` + CSS, `astro` peerDep) · `@saytu/core` config (`theme` field; `loadConfig` jiti) · Vite alias · Vitest. The mechanism is **spiked & proven** (theme-package `.astro` import; config-value-selects-theme via Vite alias).

## Global Constraints

- **No-regression is the headline gate:** the site's existing **27 render tests stay green UNCHANGED** — same HTML (shell, Post/Page templates, themed callout, prose, `lang`, zero-JS), now sourced from `@saytu/theme-default`. Success = "looks identical, now swappable."
- **`@saytu/core` change is additive config only** — an optional `theme?: string`. It must NOT touch the Markdoc converter / round-trip / content path; the existing **175 core tests + the edge guard stay green**.
...
packages/theme-default/        NEW @saytu/theme-default
...
apps/saytu-site/
  saytu.config.ts              NEW — { blocks: defaultConfig.blocks, theme: '@saytu/theme-default' }
  astro.config.mjs             MODIFIED — read theme + alias '@theme'
  package.json                 MODIFIED — + @saytu/theme-default + @saytu/core deps
...
### Task 1: `@saytu/core` — add the `theme` config field
...
Run: `pnpm --filter @saytu/core test theme-field`
...
### Task 2: Create `@saytu/theme-default` (copy the theme files; site still uses its own)
...
**Interfaces:**
- Produces: package `@saytu/theme-default` exporting `./Layout.astro`, `./PostLayout.astro`, `./PageLayout.astro`, `./theme.css`, `./site.css`.
...
git commit -m "feat(theme-default): @saytu/theme-default package (copied from the site theme)"
```
...
**Interfaces:**
- Consumes: `defineConfig`, `defaultConfig` from `@saytu/core`; `loadConfig` from `@saytu/core/node` (returns `ResolvedConfig` with `.theme` from Task 1); `@saytu/theme-default`'s exports (Task 2).
...
Run: `pnpm --filter @saytu/site test`
Expected: PASS — **27/27 unchanged** (same HTML, now sourced from `@saytu/theme-default`).

This step exercises the two things to confirm:
- **(a) `loadConfig` runs inside `astro.config.mjs`** during the build (Node + jiti — proven in #2). If the build errors loading the config (e.g. jiti can't resolve in this context), apply **fallback (b):** drop the `loadConfig` call and set `const activeTheme = '@saytu/theme-default'` directly in `astro.config.mjs` (still config-driven via the alias; wiring it to read `saytu.config` becomes a follow-on). Report if used.
...
git commit -m "feat(site): activate theme via saytu.config; render through @saytu/theme-default"
```
...
Run: `pnpm -r test`
Expected: green — `@saytu/core` 177 (175 + 2 new), `@saytu/theme-default` (no test runner — skipped/no tests), `@saytu/blocks` 8, `apps/saytu-site` 27, `apps/saytu-admin` 178, + db/git suites.
...
Expected: `scope clean` — NO markdoc/round-trip path, NO `apps/saytu-admin`, NO `@saytu/blocks` touched.

- [ ] **Step 5: Confirm the activation is real**

Run: `cat apps/saytu-site/saytu.config.ts` (shows `theme: '@saytu/theme-default'`) and confirm `apps/saytu-site/src/layouts/` no longer exists (`test -d apps/saytu-site/src/layouts && echo "STILL THERE" || echo "theme extracted ✓"`).
Expected: `theme extracted ✓` — the site renders through the config-named theme package; the layouts live only in `@saytu/theme-default`.
...
- §1 `@saytu/theme-default` package (layouts + tokens + styles): T2. ✓
- §1 `theme` field in `@saytu/core` config (additive): T1. ✓
