...
const distReadHas = (p: string, re: RegExp) =>
  existsSync(join(appDir, 'dist', p)) && re.test(readFileSync(join(appDir, 'dist', p), 'utf8'))
...
describe('SITE_CAPABILITIES matches real output', () => {
  ...
  it('file-based capabilities are accurate', () => {
    ...
    expect(SITE_CAPABILITIES.sitemapIndex).toBe(distReadHas('sitemap.xml', /<sitemapindex/))
    // post-sitemap carries Google image-extension entries for entries with images.
    expect(SITE_CAPABILITIES.imageSitemaps).toBe(distReadHas('post-sitemap.xml', /<image:image>/))
