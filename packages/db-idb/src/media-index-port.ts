...
export async function createIdbMediaIndexPort(
  dbName = 'setu-media-index'
): Promise<MediaIndexPort> {
  ...
  return {
    async query(q) {
      return runMediaQuery((await db.getAll('media')) as MediaIndexRow[], q)
    },
    async upsert(row) {
      await db.put('media', row, row.mediaKey)
    },
    async upsertMany(rows) {
      const tx = db.transaction('media', 'readwrite')
      await Promise.all([
        ...rows.map((r) => tx.store.put(r, r.mediaKey)),
        tx.done
      ])
    },
    async remove(mediaKey) {
      await db.delete('media', mediaKey)
    },
    async clear() {
    ...
    async getMeta() {
      return (
        ((await db.get('meta', 'meta')) as MediaIndexMeta | undefined) ?? {
          version: 0
        }
      )
    },
