...
packages/core/src/config/
  types.ts             MODIFIED — `theme?: string` on SetuConfig + ResolvedConfig
...
**Interfaces:**
- Produces: `SetuConfig.theme?: string`, `ResolvedConfig.theme?: string`; `resolveConfig(raw)` returns `theme` from the validated input; `loadConfig(path)` (unchanged code) therefore returns `theme` too.
...
In `interface SetuConfig`, add the field:
```ts
...
**1. Spec coverage** (against `2026-06-18-setu-theme-system-design.md`):
