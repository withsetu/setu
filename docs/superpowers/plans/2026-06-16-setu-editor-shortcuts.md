...
**Spec:** `docs/superpowers/specs/2026-06-16-setu-editor-shortcuts-design.md`
...
Append to `apps/admin/src/styles/editor.css` (reuse existing tokens; the tippy theme is scoped to `data-theme~='setu'` so it doesn't affect the slash/block popups, which use their own content):

```css
/* Shortcut tooltips (scoped tippy theme) */
.tippy-box[data-theme~='setu'] {
...
.tippy-box[data-theme~='setu'] .tippy-content { padding: 4px 8px; }
