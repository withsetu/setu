...
describe('imageMarkup', () => {
  ...
  it('emits <picture> sources (avif first) for a multi-format manifest', () => {
    ...
    const out = imageMarkup({ manifest: mf, resolvedSrc:'/o.png', alt:'a', resolveUrl:(s)=>s, sizes:'100vw' })
  ...
  it('passes lqip through and omits sources for single-format', () => {
    const mf = { id:'x', format:'webp',
      ...
      lqip:'data:image/webp;base64,AA' } as MediaManifest
    const out = imageMarkup({ manifest: mf, resolvedSrc:'/o.png', alt:'a', resolveUrl:(s)=>s, sizes:'100vw' })
