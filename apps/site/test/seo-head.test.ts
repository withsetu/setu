...
describe('SEO head emitters (#71)', () => {
  it('emits an absolute canonical + og:url for a post (mirrors the served trailing-slash URL)', () => {
    expect(post).toContain(
      '<link rel="canonical" href="https://example.com/post/astro-on-the-edge/">'
    )
    expect(post).toMatch(
      /<meta property="og:url" content="https:\/\/example\.com\/post\/astro-on-the-edge\/">/
    )
  })
  ...
  it('emits the generator + robots directives', () => {
    expect(post).toMatch(
      /<meta name="generator" content="https:\/\/setu\.build\/\?v=[^"]+">/
    )
  ...
  it('emits a JSON-LD @graph with an Article for a post (#72)', () => {
    ...
    expect(
      types.some((t: string) => t === 'Organization' || t === 'Person')
    ).toBe(true)
  })
  ...
  it('per-page seo: overrides win — title, description, noindex, canonical (#73)', () => {
    ...
    expect(override).toContain(
      '<meta name="description" content="A hand-written meta description for search.">'
    )
