...
export function MediaIndexProvider({
  service,
  children
}: {
...
export function useMediaIndex(): MediaIndexService {
  ...
  if (v === null)
    throw new Error('useMediaIndex must be used within a MediaIndexProvider')
  return v
}
...
export function AppMediaIndexProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <MediaIndexProvider service={mediaIndex}>{children}</MediaIndexProvider>
  )
}
