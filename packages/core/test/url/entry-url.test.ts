...
describe('entryUrlPath', () => {
  ...
  it('non-default locale is a LEADING prefix (changed by #251; was post/fr/…)', () => {
    expect(entryUrlPath({ collection: 'post', locale: 'fr', slug: 'bonjour' })).toBe('fr/post/bonjour')
  })
  ...
  it('a non-default-locale home is NOT the root', () => {
    expect(entryUrlPath({ collection: 'page', locale: 'fr', slug: 'home' })).toBe('fr/page/home')
  })
  it('honors a pattern config', () => {
    expect(
      entryUrlPath(
        { collection: 'post', locale: 'en', slug: 'hi', date: Date.UTC(2026, 5, 20) },
