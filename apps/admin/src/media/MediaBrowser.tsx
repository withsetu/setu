...
export const DEFAULT_SORT: MediaFilters['sort'] = {
  key: 'uploadedAt',
  dir: 'desc'
}
  ...
  return (
    <div className="flex flex-col gap-3.5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          value={sortValueOf(filters.sort)}
          onValueChange={(v) => setFilters({ sort: parseSortValue(v) })}
        >
          <SelectTrigger size="sm" aria-label="Sort" className="w-36">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {SORT_OPTIONS.map((o) => (
              <SelectItem key={`${o.key}-${o.dir}`} value={`${o.key}-${o.dir}`}>
                {o.label}
              </SelectItem>
            ))}
          </SelectContent>
