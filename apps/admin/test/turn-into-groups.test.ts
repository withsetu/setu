...
describe('TURN_INTO_GROUPS', () => {
  it('is Text(leaf), Heading(group h2/h3/h4), List(group bullet/ordered), Quote(leaf), Code(leaf)', () => {
    const shape = TURN_INTO_GROUPS.map((e) =>
      e.kind === 'leaf'
        ? `leaf:${e.type.id}`
        : `group:${e.id}[${e.items.map((i) => i.id).join(',')}]`
    )
...
describe('checklist block type', () => {
  ...
  it('adds Checklist as the third item of the List group', () => {
    const list = TURN_INTO_GROUPS.find(
      (e) => e.kind === 'group' && e.id === 'list'
    )
    expect(
      list && list.kind === 'group' && list.items.map((i) => i.id)
    ).toEqual(['bulletList', 'orderedList', 'taskList'])
  })
})
