...
function Counter({ len, max }: { len: number; max: number }) {
  return (
    <span className={`text-[11px] tabular-nums ${len > max ? 'text-amber-600' : 'text-muted-foreground'}`}>
      {len}/{max}
    </span>
  )
}
...
export function SeoSection({
  ...
  const seo = (metadata['seo'] && typeof metadata['seo'] === 'object' ? metadata['seo'] : {}) as SeoOverride
  const docTitle = typeof metadata['title'] === 'string' ? (metadata['title'] as string) : ''
  const docDesc =
    (typeof metadata['description'] === 'string' && (metadata['description'] as string)) ||
    (typeof metadata['summary'] === 'string' && (metadata['summary'] as string)) ||
    ''
  ...
  const previewDesc =
    resolved.meta.find((m) => m.name === 'description')?.content || 'Add a meta description to control the search snippet.'
  ...
  return (
    <div className="space-y-3.5">
      ...
      <div className="rounded-md border border-border/60 bg-muted/30 px-3 py-2.5">
        <div className="truncate text-[12px] text-emerald-700 dark:text-emerald-500">{previewUrl}</div>
        <div className="truncate text-[15px] leading-snug text-blue-700 dark:text-blue-400">{previewTitle}</div>
        <div className="mt-0.5 line-clamp-2 text-[12px] leading-snug text-muted-foreground">{previewDesc}</div>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder={docDesc || 'Falls back to the excerpt, then the site description.'}
          disabled={!editable}
        />
      </div>
        ...
        <div className="space-y-3.5 border-t border-border/60 pt-3">
          <div className="flex items-center justify-between gap-3">
            <div className="space-y-0.5">
              ...
              <p className="text-[11px] text-muted-foreground">Emits <code>noindex, nofollow</code> for this page.</p>
            </div>
            <Switch
              id="seo-noindex"
              checked={!!seo.noindex}
