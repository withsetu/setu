...
describe('parseHealthState', () => {
  it('defaults to empty on missing/malformed input (never throws)', () => {
    ...
    expect(parseHealthState('not an object')).toEqual({
      items: {},
      sections: {}
    })
  ...
  it('keeps well-formed records and drops malformed ones', () => {
    const s = parseHealthState({
      items: {
        'a.b': { state: 'attested', at: '2026-01-01', by: 'Local' },
        bad: { state: 'nope' }
      },
      sections: { i18n: { state: 'na', at: '2026-01-01', by: 'Local' } }
    })
...
describe('setHealthRecord', () => {
  it('sets and clears item records immutably', () => {
    const a = setHealthRecord({ items: {}, sections: {} }, 'item', 'x', {
      state: 'na',
      at: '2026-01-01',
      by: 'Local'
    })
