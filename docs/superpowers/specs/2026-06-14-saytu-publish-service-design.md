# Design — Publish Service (`@saytu/core`) (Increment #6)
...
- Export the service + `contentPath` + types from `@saytu/core`.
...
incrementing sha + head ref, like `@saytu/git-testing`'s fake).
