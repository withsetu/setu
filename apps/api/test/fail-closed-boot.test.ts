...
function makeDegradedApp() {
  ...
  app.use('*', authUnconfiguredGuard(() => secret === null))
  ...
  app.on(['POST', 'GET'], '/api/auth/*', (c) => c.json({ error: 'unreachable: guard should have short-circuited' }, 500))

  app.route('/', createUploadApi({ storage: memStorage(), resolveActor: () => null }))
  app.route(
    '/',
    createCapabilitiesApi(
      buildCapabilities({ writableMediaStore: true, backgroundJobs: true, mode, auth: { enabled: false, providers: [], captcha: null, needsSetup: false } }),
      () => ({ enabled: false, providers: [], captcha: null, needsSetup: false }),
    ),
  )
...
describe('fail-closed boot degradation (non-local mode, no SETU_AUTH_SECRET)', () => {
  it('POST /media -> 503 with the exact "auth not configured" body (not the auth middleware\'s 401, not a crash)', async () => {
    ...
    const res = await app.fetch(
      new Request('http://test/media', { method: 'POST', headers: { origin: TRUSTED_ORIGIN }, body: form }),
    )
    expect(res.status).toBe(503)
    expect(await res.json()).toEqual({ error: 'auth not configured', hint: 'set SETU_AUTH_SECRET' })
  })
  ...
  it('POST /api/auth/sign-in/email -> 503 (the auth mount is short-circuited before it ever runs)', async () => {
    ...
    const res = await app.fetch(
      new Request('http://test/api/auth/sign-in/email', {
        ...
        body: JSON.stringify({ email: 'a@example.com', password: 'whatever123' }),
      }),
    )
    expect(res.status).toBe(503)
    expect(await res.json()).toEqual({ error: 'auth not configured', hint: 'set SETU_AUTH_SECRET' })
...
describe('resolveActor fails closed when auth is unconfigured (503, not 401 — see comment in resolve-session-actor.ts)', () => {
  it('a gated route reachable in a mixed topology still 503s rather than misreport 401', async () => {
    ...
    try {
      ...
      const auth = createAuth({ db, secret: 'configured-secret-for-comparison-only', baseURL: 'http://localhost:4444', trustedOrigins: [] })
      const resolveActor = resolveSessionActor(auth)
      const result = await resolveActor(new Request('http://test/', { headers: { cookie: 'better-auth.session_token=garbage' } }))
