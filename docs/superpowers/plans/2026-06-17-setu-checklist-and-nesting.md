...
In `apps/admin/src/editor/styles/editor.css`, after the existing `.setu-prose li::marker` rule (~line 120), add:

```css
/* ---- Task list (checklist) ---- */
.setu-prose ul[data-type="taskList"] { list-style: none; padding-left: 4px; margin: 6px 0; }
.setu-prose ul[data-type="taskList"] li[data-type="taskItem"] { display: flex; align-items: flex-start; gap: 9px; padding: 1px 0; }
.setu-prose li[data-type="taskItem"] > label { flex-shrink: 0; margin-top: 7px; user-select: none; }
.setu-prose li[data-type="taskItem"] > label > input[type="checkbox"] { width: 16px; height: 16px; cursor: pointer; accent-color: var(--accent-strong); }
.setu-prose li[data-type="taskItem"] > div { flex: 1; min-width: 0; }
.setu-prose li[data-type="taskItem"] > div > p { margin: 0; padding: 0; }
.setu-prose li[data-type="taskItem"][data-checked="true"] > div { color: var(--text-3); text-decoration: line-through; }
/* nested lists keep their own marker style and indent under the content column */
.setu-prose li[data-type="taskItem"] ul[data-type="taskList"] { padding-left: 0; }
```
